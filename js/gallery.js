...
(function(root, factory) {
  ...
  var previousButtonClickHandler = function(event) {
    event.stopPropagation
      ? event.stopPropagation()
      : (event.cancelBubble = true); // jshint ignore:line
    showPreviousImage();
  };
  ...
  function run(selector, userOptions) {
    ...
    testWebpSupport(function(result) {
      ...
      [].forEach.call(tagsNodeList, function(imageElement, imageIndex) {
        var imageElementClickHandler = function(event) {
          event.preventDefault
            ? event.preventDefault()
            : (event.returnValue = false); // jshint ignore:line
  ...
  function setOptions(newOptions) {
    ...
    slider.style.transition = slider.style.webkitTransition = options.animation ===
      "fadeIn"
      ? "opacity .4s ease"
  ...
  function getImageSrc(image) {
    ...
    if (image.dataset) {
      var srcs = [];
      var validAttribute = /(at-|webpAt-)(\d+)/i;
      var webpTest = /webp/i;

      // Get all possible image versions depending on the resolution
      for (var item in image.dataset) {
        if (validAttribute.test(item)) {
          var key = item.replace(validAttribute, "$2");
          var src = image.dataset[item];
          var isWebp = webpTest.test(item);

          if (supports.webp) {
            //if a webp and non-webp option are given for the same size, prioritize webp
            if (!srcs[key] || (isWebp && srcs[key].search(/webp/i) == -1)) {
              srcs[key] = src;
            } 
          } else if (!isWebp) {
            //browser doesn't support webp so we'll filter those out
            srcs[key] = src;
          }
        }
      }
  ...
  // Inline WebP test (lossy)
  function testWebpSupport(callback) {
    var webp = new Image();
    webp.src =
      "data:image/webp;base64,UklGRiIAAABXRUJQVlA4IBYAAAAwAQCdASoBAAEADsD+JaQAA3AAAAAA";
    webp.onload = function() {
      callback(webp.width > 0 && webp.height > 0);
    };
